...
parser.add_argument('--voc_root', default='/home/pengwu/data/VOCdevkit/',
...
def write_voc_results_file(all_boxes, dataset):
    for cls_ind, cls in enumerate(labelmap[1:]):
...
def test_net(save_folder, net, priors, cuda, dataset, transform, top_k,
    ...
    for i in range(num_images):
        ...
        for c in range(1, cfgs.model['num_classes']):
            ...
            boxes_c[:, 1] *= h
            boxes_c[:, 3] *= h
            cls_dets = np.hstack((boxes_c,
                                  scores_c[:, np.newaxis])).astype(np.float32,
                                                                 copy=False)
            all_boxes[c][i] = cls_dets
...
if __name__ == '__main__':
    ...
    dataset = VOCDetection(args.voc_root, 
                           [('2007', set_type)],
                           BaseTransform(tuple(cfgs.coder.input_shape), cfgs.train.aug_params.bgr_mean))
    if args.cuda:
        net = net.cuda()
        cudnn.benchmark = True
    # evaluation
    test_net(args.save_folder, net, priors, args.cuda, dataset,
             None, args.top_k, 300,
